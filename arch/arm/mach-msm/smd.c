...
void share_mmi_unit_info(struct platform_device *pdev)
{
	...
	if (!pdata->custom_data) {
		pr_err("%s: platform custom_data not initialized\n",
			__func__);
		return;
	}
	...
	SMD_INFO("%s: mmi_unit_info version = %d and size = %d\n",
		__func__,
		((struct mmi_unit_info_v1 *)pdata->custom_data)->version,
	...
	memcpy(smui, pdata->custom_data, sizeof(struct mmi_unit_info_v1));
