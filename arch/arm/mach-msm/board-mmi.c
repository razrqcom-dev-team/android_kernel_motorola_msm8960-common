		...
		struct clk *clk_entity, const char *dev_id)
{
	...
	if (!msm_clocks_8960_common_info(&lk, &num_lookups)) {
		int i;
...
bool mipi_mot_panel_is_cmd_mode(void)
{
	...
	if (!strncmp("mipi_mot_video_smd_hd_465", panel_name, PANEL_NAME_MAX_LEN))
		ret = false;
...
static void __init init_mmi_unit_info(void) {
...
// FIXME-HASH: need to fix extension to ram_console
...
static void __init msm8960_fixup(struct tag *tags, char **cmdline,
				 struct meminfo *mi)
{
	struct	tag	*t;
